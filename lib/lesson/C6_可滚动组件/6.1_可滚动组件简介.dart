...
///在本章中，我们先介绍一下常用的可滚动组件（如ListView、GridView等），
///然后介绍一下ScrollController。可滚动组件都直接或间接包含一个Scrollable组件，
///因此它们包括一些共同的属性，为了避免重复介绍，我们在此统一介绍一下：
/*
Scrollable({
  ...
...
// 可以显式指定一个固定的ScrollPhysics，FlutterSDK中包含了两个ScrollPhysics的子类，他们可以直接使用：
...
// 我们将在本章后面“滚动控制”一节详细介绍ScrollController。

///Scrollbar
//Scrollbar是一个Material风格的滚动指示器（滚动条），
// 如果要给可滚动组件添加滚动条，只需将Scrollbar作为可滚动组件的任意一个父级组件即可，如：
/*
Scrollbar(
...
///ViewPort视口
//在很多布局系统中都有ViewPort的概念，在Flutter中，术语ViewPort（视口），
//如无特别说明，则是指一个Widget的实际显示区域。
//例如，一个ListView的显示区域高度是800像素，
...
//如果一个可滚动组件支持Sliver模型，那么该滚动可以将子组件分成好多个“薄片”（Sliver），
//只有当Sliver出现在视口中时才会去构建它，这种模型也称为“基于Sliver的延迟构建模型”。
//可滚动组件中有很多都支持基于Sliver的延迟构建模型，
